...
export async function createSemeFiller(
  ...
  const patternNode = addPattern(builder.defs, id, w, h, patternTransform);
...
function createChargeTransforms(charge: ChargeVisualInfo, tx: number, ty: number) {
...
function computeTransform(
  ...
  if (container.type == "strip") {
    ...
    if (container.stripDirection == "fasce") {
      scale = computeSemeScale(container.stripWidth, semeHeight, chargeFullHeight);
    } else {
      scale = computeSemeScale(container.stripWidth, semeWidth, chargeFullWidth);
    }
    ...
    const scale = computeSemeScale(container.stripWidth, semeWidth, chargeFullWidth);
...
function computeSemeScale(targetSize: number, patternSize: number, fullSize: number): number {
